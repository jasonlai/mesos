...
TEST_F(ProvisionerDockerLocalStoreTest, PruneImages)
{
  ...
  vector<Image> activeImages{image};

  // PruneImages with same activeImages call should
  // keep the layers around.
  Future<Nothing> prune = store.get()->prune(activeImages, {});
  AWAIT_READY(prune);
  ...
  // in a empty layers directory.
  Future<Nothing> prune3 = store.get()->prune({}, {});
  AWAIT_READY(prune3);

  EXPECT_FALSE(os::exists(layerPath1));

  // We can still get the image, but it
  // will require the store to pull again.
