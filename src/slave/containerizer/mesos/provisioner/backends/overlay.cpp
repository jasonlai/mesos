    ...
    const string& backendDir)
{
  ...
  if (symlink.isError()) {
    return Failure(
        ...
        "' to temporary directory '" + tempDir + "'");
  }

  VLOG(1) << "Created symlink at '" << tempLink << "' to '" << tempDir << "'";
  ...
  foreach (const fs::MountInfoTable::Entry& entry, mountTable->entries) {
    if (entry.target == rootfs) {
      ...
      if (!os::exists(tempLink)) {
        ...
        LOG(WARNING)
            << "Cannot find symlink to temporary directory '" << tempLink
            <<"' for image links.";
      } else if (!os::stat::islink(tempLink)) {
        return Failure(
        ...
        if (realpath.isSome()) {
          ...
          if (rmdir.isError()) {
            return Failure("Failed to remove temporary directory '" +
                           realpath.get() + "' pointed by '" +
                           tempLink + "': " + rmdir.error());
          }
