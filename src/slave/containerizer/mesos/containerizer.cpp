    ...
    const Option<state::SlaveState>& state)
{
  ...
  foreach (const ContainerState& state, recoverable) {
    ...
    if (config.isError()) {
      return Failure(
        "Failed to get container config for container " +
        stringify(containerId) + ": " + config.error());
    }
    ...
    if (config.isSome()) {
      ...
      LOG(INFO) << "No container config is recovered for container "
                << containerId << ", image pruning will be disabled";
    }
  ...
  foreach (const ContainerID& containerId, containerIds.get()) {
    ...
    if (pid.isError()) {
      return Failure(
          "Failed to get pid for container " + stringify(containerId) +
          ": " + pid.error());
    }
    ...
    if (config.isSome()) {
      ...
      LOG(INFO) << "No checkpointed container config recovered for container "
                << containerId << ", image pruning will be disabled";
    }
