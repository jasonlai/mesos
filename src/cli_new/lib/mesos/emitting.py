...
from pygments.formatters import Terminal256Formatter  # pylint: disable=no-name-in-module,ungrouped-imports
from pygments.lexers import JsonLexer  # pylint: disable=no-name-in-module,ungrouped-imports

from mesos import config, constants, exceptions, util
...
def print_handler(event):
    ...
    :type event: str, dict, list, or dcos.exceptions.Error
    """

    pager_command = os.environ.get(constants.MESOS_PAGER_COMMAND_ENV)
    ...
    elif isinstance(event, exceptions.Error):
    ...
    elif isinstance(event, exceptions.MesosException):
...
def _page(output, pager_command=None):
    ...
    if exceeds_tty_height and paginate and \
            find_executable(pager_command.split(' ')[0]) is not None:
