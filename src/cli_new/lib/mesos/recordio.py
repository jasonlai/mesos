...
class Encoder(object):
    ...
    def encode(self, message):
        ...
        smsg = self.serialize(message)

        if not isinstance(smsg, bytearray):
            raise MesosException("Calling 'serialize(message)' must"
                                 " return a 'bytearray' object")

        return bytearray(str(len(smsg)) + "\n", "UTF-8") + smsg
...
class Decoder(object):
    ...
    def decode(self, data):
        ...
        if not isinstance(data, bytearray):
            raise MesosException("Parameter 'data' must of of type 'bytearray'")
        ...
        for char in data:
            if self.state == self.HEADER:
                ...
                try:
                    ...
                    raise MesosException("Failed to decode length '{buffer}':"
                                         " {error}".format(buffer=self.buffer,
                                                           error=exception))
